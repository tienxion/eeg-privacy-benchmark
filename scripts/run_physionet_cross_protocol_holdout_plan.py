...
DEFAULT_PLAN_CSV = (
    ROOT / "outputs" / "reports" / "physionet_cross_protocol_holdout_plan_v1.csv"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run planned rows from the PhysioNet cross-protocol holdout plan."
    )
...
def main() -> None:
    ...
    log_path = LOG_ROOT / f"physionet_cross_protocol_holdout_{timestamp}.log"

    print(f"Loaded {len(commands)} planned cross-protocol holdout commands")
